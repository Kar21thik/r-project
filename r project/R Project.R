...
movie_data <- read.csv("~/Custom Office Templates/movies.csv")
rating_data <- read.csv("~/Custom Office Templates/ratings.csv")
...
# Visualizing the distribution of the average ratings per user   main code 000000
...
movie_data
barchart<-ggplot()
barchart<- barchart+geom_col(data=movie_data,aes(x=genre,y=number,fill=genre),position = "dodge")
barplot
...
library(stringi)
movie_data$title<-gsub("A^","",as.character(factor(movie_data$title)))
stri_trim(movie_data$title,side="right")
...
genres.df <- as.data.frame(movie_data[,c("genres", "title")])
...
means <- rep(0,23)
for (i in 1:23) {
  means[i] <- mean(genres.df$title[genres.df[i+2]==1])
}
# plot the means
...
ggplot(movie_data, aes(title)) +
  geom_bar() +
  labs(x = "Year movie was released", y = "Movie Count", title = "Histogram of Movie released") +
  theme(plot.title = element_text(hjust = 0.5))
movie_data<- movie_data[movie_data$title >= 1990,]


plot(movie_data,type="o")
